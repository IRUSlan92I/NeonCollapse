...
@export var jump_another_direction : AbstractPlayerState
...
func enter() -> void:
	...
	match direction:
		Direction.Left:
			player.player_sprite.play(ANIMATION_RUN_LEFT)
			player.blade_sprite.play(ANIMATION_RUN_LEFT)
		Direction.Right:
			player.player_sprite.play(ANIMATION_RUN_RIGHT)
			player.blade_sprite.play(ANIMATION_RUN_RIGHT)
			...
			match direction:
				Direction.Left:
					if player.velocity.x < 0.0:
						...
						switch_state.emit(jump_another_direction)
				Direction.Right:
					if player.velocity.x > 0.0:
						switch_state.emit(jump)
					else:
